...
void mpu_config_bootloader(void)
{
    ...
    // Flash (0x0800C000 - 0x0800FFFF, 16 KiB, read-write, execute never) - for calibration data
    MPU->RNR = MPU_REGION_NUMBER1;
    MPU->RBAR = FLASH_BASE + 0xC000;
    MPU->RASR = MPU_RASR_ENABLE_Msk | MPU_RASR_ATTR_FLASH | LL_MPU_REGION_SIZE_16KB | LL_MPU_REGION_FULL_ACCESS | MPU_RASR_XN_Msk;
    ...
    // STM32F429I-DISC1: SDRAM at 0xD0000000 (FMC bank 2, 8 MiB, read-write, execute never)
    MPU->RNR = MPU_REGION_NUMBER5;
    MPU->RBAR = 0xD0000000;
    MPU->RASR = MPU_RASR_ENABLE_Msk | MPU_RASR_ATTR_SRAM | LL_MPU_REGION_SIZE_8MB | LL_MPU_REGION_FULL_ACCESS | MPU_RASR_XN_Msk;
...
void mpu_config_firmware(void)
{
    ...
    // Note: later entries overwrite previous ones (higher region numbers have priority)

    // Background: Everything (0x00000000 - 0xFFFFFFFF, 4 GiB, read-write, execute never)
    // This provides default access; specific regions below provide restrictions/permissions
    // Needed for calibration data at 0x0800C000 and other memory-mapped areas
    MPU->RNR = MPU_REGION_NUMBER0;
    MPU->RBAR = 0;
    MPU->RASR = MPU_RASR_ENABLE_Msk | MPU_RASR_ATTR_PERIPH | LL_MPU_REGION_SIZE_4GB | LL_MPU_REGION_FULL_ACCESS | MPU_RASR_XN_Msk;
    ...
    // Peripherals (0x40000000 - 0x5FFFFFFF, read-write, execute never)
    // External RAM (0x60000000 - 0x7FFFFFFF, read-write, execute never)
    // Note: covered by background region 0, specific region not needed

#if defined(STM32F429xx)
    // STM32F429I-DISC1: SDRAM at 0xD0000000 (FMC bank 2, 8 MiB, read-write, execute never)
    // Note: covered by background region 0, specific region not needed for basic access
